...
//    }
    ...
    public ArrayList<myproduct> getProduct(int id) {
        ArrayList<myproduct> pro = new ArrayList<>();
        con = db.setConnection();
        try {
            String sql = "Select * from pro.myproduct where ID_category = '" + id + "'";
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
                ...
                temp.setID_manufacturer(rs.getInt("ID_supplier"));
    ...
    public myproduct productDetail(int id) {
        ...
        con = db.setConnection();
        try {
            String sql = "Select * from pro.myproduct where ID_product ='" + id + "'";
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
    ...
    public boolean order(shoppingcart1 cart, String accid) {
        ...
        int year = localDate.getYear(); // lay nam
        int month = localDate.getMonthValue(); // lay thang
        int day = localDate.getDayOfMonth(); // lay ngay
        
        con = db.setConnection();
        try {
            // insert order
            String sql = "Insert Into pro.order (accid, day, month, year, totalPrice) values(? , ? , ? , ? , ?)";
            PreparedStatement ps = con.prepareStatement(sql);
            ...
            //
            // lay id_order
            String sql2 = "SELECT max(ID_order) as max FROM pro.`order` where accid = '" + accid + "'  and day = '" + day + "' and month = '" + month + "' and year = '" + year + "'and totalPrice ='" + cart.getTotal() + "';";
            ...
            for (int j = 0; j < list.size(); ++j) {
                ...
                String sql3 = "Insert Into pro.orderproduct (ID_order, ID_product, quantity, price, totalPrice ) values(? , ? , ? , ? , ?)";
    ...
    public ArrayList<order> getOrder(int day, int month, int year) {
        ...
        con = db.setConnection();
        try {
            String sql = "SELECT * FROM pro.`order` where day = '" + day + "' and  month = '" + month + "' and year = '" + year + "'";
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
    ...
    public ArrayList<order> getOrder1(int month, int year) {
        ...
        con = db.setConnection();
        try {
            String sql = "SELECT * FROM pro.`order` where  month = '" + month + "' and year = '" + year + "'";
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
                order temp = new order();
                ...
                String sql1 = "SELECT * FROM pro.orderproduct where ID_order ='" + id + "'";
                ResultSet rs1 = db.getResult(sql1, con);
                ArrayList<myproduct> cart = temp.getList();
                while (rs1.next()) {
                    myproduct temp1 = new myproduct();
    ...
    public ArrayList<order> getOrderID(int ID_oder) {
        ...
        con = db.setConnection();
        try {
            String sql = "SELECT * FROM pro.`order` where  ID_order = '" + ID_oder + "'";
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
                order temp = new order();
                ...
                String sql1 = "SELECT * FROM pro.productlist where ID_order ='" + id + "'";
                ResultSet rs1 = db.getResult(sql1, con);
                ArrayList<myproduct> cart = temp.getList();
                while (rs1.next()) {
                    myproduct temp1 = new myproduct();
    ...
    public ArrayList<order> getOrderAcc(String accid) {
        ...
        con = db.setConnection();
        try {
            String sql = "SELECT * FROM pro.`order` where  accid = '" + accid + "'";
            System.out.println(sql);
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
    ...
    public order getOrder(String ID_order) {
        ...
        con = db.setConnection();
        try {
            String sql = "SELECT * FROM pro.`order` where  ID_order = '" + ID_order + "'";
            System.out.println(sql);
            try (ResultSet rs = db.getResult(sql, con)) {
                int i = 0;
                while (rs.next()) {
    ...
    public ArrayList<myproduct> getProductSearch(String name) {
        ...
        con = db.setConnection();
        try {
            String sql = "Select * from pro.myproduct where name Like '" + name + "%'";
            System.out.println(sql);
            ResultSet rs = db.getResult(sql, con);
            int i = 0;
            while (rs.next()) {
    ...
    public void uploadProduct(String name, String description, String price, String ID_supplier, String ID_status, String ID_category, String[] url) {
        double price1 = Double.parseDouble(price);
        System.out.println(price1);
        con = db.setConnection();
        try {
            String sql = "Insert Into pro.myproduct (name, description, price, ID_supplier, ID_status, ID_category) values(? , ? , ? , ? , ?,?)";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, name);
            ps.setString(2, description);
            ps.setDouble(3, price1);
            ps.setInt(4, Integer.parseInt(ID_supplier));
            ps.setInt(5, Integer.parseInt(ID_status));
            ps.setInt(6, Integer.parseInt(ID_category));

            int result = ps.executeUpdate();
            System.out.print(sql);
            //

            String sql4 = "SELECT * FROM pro.myproduct where name ='" + name + "' and price = '" + price + "' and description = '" + description + "' and ID_supplier = '" + ID_supplier + "' and ID_status ='" + ID_status + "' and ID_category ='" + ID_category + "'";
            PreparedStatement stmt = con.prepareStatement(sql4);
            //stmt.setString(1, term + "%");
            ResultSet rs4 = stmt.executeQuery();
            //ResultSet rs4 = db.getResult(sql4, con);
            ...
            //System.out.print("jujuj " + abc);
            while (rs4.next()) {
                ID_product = rs4.getInt("ID_product");
            }
            PreparedStatement ps2 = null;
            System.out.println(ID_product);

            for (int j = 0; j < url.length; ++j) {
                System.out.print(url[j]);
                String sql1 = "Insert Into pro.url(ID_product, url) values(?,?)";
                ps2 = con.prepareStatement(sql1);
                ps2.setInt(1, ID_product);
                ps2.setString(2, url[j]);
